...
### Recommendations
There are further factors not contained in our dataset that can have an impact on an individual such a drug use, alcohol consumption and family history.  Collecting and analyzing these additional factors would help improve our machine learning model.  Additionally, our dataset didn't have a high number of stroke victims so we had to oversample our dataset so the number of stroke occurences equaled the number of non-stroke occurences.  Finding a dataset that had more individuals who had a stroke would be beneficial moving forward.  
